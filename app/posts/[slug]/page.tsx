...
function cleanLine(line: string): string {
  ...
  while (true) {
    const next = s.replace(/^\s*(?:[-–—•·●◦*]+)\s*/u, "").trimStart();
  ...
  return (
    <>
      ...
      <p className="mt-4 text-[10px] font-semibold uppercase tracking-[0.2em] text-zinc-500">
        {post.category.toUpperCase()} · {post.minutes} min read
      </p>
      ...
      {post.readMore && (
        <section className="mt-10 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="mt-2 inline-block break-all text-sm font-medium text-lime-600 underline hover:text-lime-700"
          >
            {post.readMore}
          </a>
        </section>
